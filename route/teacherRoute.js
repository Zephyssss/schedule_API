...
router.post("/", async (req, res) => {
  ...
  if (find)
    return res.status(409).json({ error: "Conflict", message: "Teacher_id exist" });
  ...
  if(!checkGradeSubject)
    return res.status(409).json({error:"Conflict", message:"Grade-Subject don't exist"})
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!findTeacher)
      res.status(404).json({ error: "Not found", message: "Teacher not found" });
...
router.put("/:id", async (req, res) => {
  ...
  const valid = await validate.updateTeacherValidation({id: req.params.id,...req.body,});
  if (valid.error)
    return res.status(400).json({ error: "Bad request", message: valid.error.details[0].message });
  ...
  if (req.body.teacher_id != undefined) {
    if (req.body.teacher_id.trim() != findTeacher.teacher_id.trim()) {
      const find = await Teacher.findOne({id_user: req.user._id,teacher_id: req.body.teacher_id.trim(),});
      if (find)
        return res.status(409).json({ error: "Conflict", message: "Teacher_id is exist" });
  ...
  if (req.body.subject != undefined) findTeacher.subject = req.body.subject.trim();
